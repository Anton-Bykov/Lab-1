...
with open("adc_data_Anton_Vosbujden.txt", "r") as file:
    array_rest_pressure_1 = [Pressure(int(i)) for i in file.read().split("\n")]

array_rest_pressure = []
for i in range(0, len(array_rest_pressure_1)):
...
aproxim = savgol_filter(array_rest_pressure, 100, 1)
...
for i in range (0, len(array_rest_pressure)):
    if (array_time[i] > 13.6 and array_time[i] < 14.4) :
        print(array_rest_pressure[i])
...
ax.set_title("\n".join(wrap('Артериальное давление в возбужденном состоянии', 80)), fontsize = 25, pad = 20, loc = 'center')
...
ax.plot(data_time, data, c='orange', linewidth=2, label ='Давление - ???/??? [мм рт.ст.]')
...
ax.text(13.76, 0.78, 'a = 1.7 мм рт.ст; t(systole) = 0.32 с', rotation = 0, fontsize = 24)
ax.text(13.76, 0.53, 'b = 1.125 мм рт.ст; t(diastole) = 0.3 с', rotation = 0, fontsize = 24)
ax.text(13.76, 0.25, 'dt = 0.26 c', rotation = 0, fontsize = 24)

#сохранение
fig.savefig('rest-pressure-Fitness.png')